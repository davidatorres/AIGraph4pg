# AIGraph4pg - Documentation Index

## External Links within this repo

### AIGraph4pg reference application

- [Quick Start](quick_start.md)
- [Frequently Asked Questions (FAQ)](faq.md)
- [Environment Variables](environment_variables.md)

### PostgreSQL and psql

- [psql notes](psql_notes.md)
- [psql help](psql_help.md)
- [pg_dump help](pg_dump_help.md)
- [pg_restore help](pg_restore_help.md)
- [postgresql local win11 setup](postgresql_local_win11_setup.md)

### Python Topics

- [Azure PostgreSQL Extensions](pg_extensions.md)
- [python relational database access](python_db_access.md)

### Apache Age Topics

- [The Apache Age Extension](age.md)

---

## External Links outside of this repo


### Azure PostgreSQL and pgvector 

- https://learn.microsoft.com/en-us/azure/postgresql/flexible-server/how-to-use-pgvector
- https://learn.microsoft.com/en-us/azure/postgresql/flexible-server/how-to-optimize-performance-pgvector
- https://learn.microsoft.com/en-us/azure/postgresql/flexible-server/how-to-optimize-performance-pgvector#indexing

### Azure PostgreSQL and Apache AGE

- https://techcommunity.microsoft.com/t5/azure-database-for-postgresql/introducing-support-for-graph-data-in-azure-database-for/ba-p/4275628

### PostgreSQL SDKs

- https://learn.microsoft.com/en-us/azure/postgresql/flexible-server/connect-python
- pip install psycopg2
- pip install azure-identity

### Apache AGE and openCypher graph

- https://age.apache.org/
- https://age.apache.org/getstarted/quickstart/
- https://github.com/apache/age
- https://github.com/apache/age-viewer
- https://python.langchain.com/docs/integrations/graphs/apache_age/
- https://www.linkedin.com/pulse/apache-age-bridging-relational-databases-graphs-frank-wk5he/
- https://opencypher.org/references/
- https://opencypher.org/references/#gql
- https://s3.amazonaws.com/artifacts.opencypher.org/openCypher9.pdf
- https://s3.amazonaws.com/artifacts.opencypher.org/website/materials/DM32.2/DM32.2-2018-00126.The.GQL.Manifesto.pdf
- https://pypi.org/project/apache-age-python/
- https://github.com/apache/age/tree/master/drivers/python

### openCypher GenAI

- https://towardsdatascience.com/evaluating-llms-in-cypher-statement-generation-c570884089b3
- https://python.langchain.com/docs/integrations/graphs/apache_age/
- https://medium.com/neo4j/generating-cypher-queries-with-chatgpt-4-on-any-graph-schema-a57d7082a7e7

### Azure Samples

- https://github.com/Azure-Samples/rag-postgres-openai-python
- https://github.com/Azure-Samples/Azure-PostgreSQL-Resilience-Architecture
- https://github.com/Azure-Samples/linux-postgres-migration
- https://github.com/Azure-Samples/azure-postgres-pgvector-python
- https://github.com/Azure-Samples/msdocs-fastapi-postgresql-sample-app
- https://github.com/Azure-Samples/azure-fastapi-cosmos-postgres-aca

### Semantic Kernel

- https://learn.microsoft.com/en-us/semantic-kernel/overview/

### Prompt Flow

- https://learn.microsoft.com/en-us/azure/ai-studio/how-to/prompt-flow
- https://microsoft.github.io/promptflow/index.html
- https://pypi.org/project/promptflow/
- https://github.com/microsoft/promptflow
- https://marketplace.visualstudio.com/items?itemName=prompt-flow.prompt-flow

