...
def print_options(msg):
    ...
    """
    logging.info("{} {}".format(os.path.basename(__file__), msg))
...
def get_pg_connection_str():
    ...
    # if password is empty, get a token using AzureCliCredential
...
async def create_libraries_table_vector_index(pool: psycopg_pool.AsyncConnectionPool):
    ...
    index_filename = "python/sql/libraries_ivfflat_index.sql"
    logging.info(
    ...
    try:
        ...
        if len(sys.argv) < 2:
            ...
            if func == "log_defined_env_vars":
                ...
                await delete_define_table(pool, "python/sql/libraries_ddl.sql", "libraries")
